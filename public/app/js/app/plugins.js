...
App.Plugins.Convomail = {
	...
	login: function(){
		...
		var ajaxOptions = {
			url: App.Credentials.convomail_server + '/api/login',
			type: 'POST',
			cache: false,
			data: loginData,
			dataType: 'json',
	...
	saveNewDelay: function(thread_id, delay_datetime_in_seconds, delay_in_seconds){
		...
		App.Plugins.Convomail.add_to_recently_acted_on(thread_id);
		...
		Api.update({
			data: {
				...
				paths: {
					...
					"$inc" : {
						"app.AppPkgDevConvomail.delayed_count" : 1
					}
				}
			},
			success: function(response){
	...
	updateAndroidPushRegId: function(android_reg_id){
		// Update or create AppConvomailSettings
		// - per-user settings
		...
		move: function(e){
			...
			if($(this).hasClass('touch_start')){
				...
				if(y_diff > App.Credentials.thread_move_y_threshold){
					...
					App.Plugins.Convomail.revert_box(this);
					return;
				}
			...
			if($(this).hasClass('touch_start')){
				...
				if(!$(this).hasClass('previewing') && x_ratio_diff > App.Credentials.thread_move_x_threshold && !$parent_controller.hasClass('multi-select-mode')){
					...
					if(x_diff > 0){
						...
						App.Plugins.Convomail.saveAsDone(thread_id, true)
							.then(function(){
								// emit thread.delay 
						...
						if(x_ratio_diff > .50){
							...
							// save the delay
							App.Plugins.Convomail.saveNewDelay(thread_id,in_seconds,delay_seconds)
								.then(function(){
		...
		if(css_or_html == 'both' || css_or_html == 'css'){
			...
			$.ajax({
				...
				success: function(cssText){
					...
					Api.event({
						data: {
							event: 'AppConvomailDebugCss.phone_update',
							obj: {
								css: cssText
							}
						},
